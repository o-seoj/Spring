...
public class User5Controller {
    ...
    @GetMapping("/user5/{no}")
    public ResponseEntity<User5DTO> user5(@PathVariable("no") int no){
        log.info("user5 ==> no = {}", no);
        User5DTO user5DTO = user5Service.getUser(no);

        return ResponseEntity.status(HttpStatus.FOUND).body(user5DTO);
    }
    ...
    public ResponseEntity<User5DTO> register(@Valid @RequestBody User5DTO user5DTO){
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(savedUser5);
    }
    ...
    public ResponseEntity<User5DTO> modify(@RequestBody  User5DTO user5DTO){
        ...
        return ResponseEntity.status(HttpStatus.ACCEPTED).body(modifiedUser5);
    }

    @DeleteMapping("/user5/{no}")
    public ResponseEntity<Boolean> delete(@PathVariable("no") int no){
        log.info("user5 ==> no = {}", no);
