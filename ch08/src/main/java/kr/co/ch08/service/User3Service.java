...
public class User3Service {
    ...
    public List<User3DTO> getUserAll(){
        ...
        return list.stream().map(entity -> entity.toDTO()).toList();
    }
    public User3DTO modify(User3DTO user3DTO) {
        return user3Repository.findById(user3DTO.getUserid())
                .map(user -> {
                    User3 updated = User3.builder()
                            .userid(user.getUserid())
                            .name(user3DTO.getName())
                            .role(user3DTO.getRole())
                            .regDate(LocalDateTime.now())
                            .build();
                    return user3Repository.save(updated).toDTO();
                })
                .orElse(null);
    }
